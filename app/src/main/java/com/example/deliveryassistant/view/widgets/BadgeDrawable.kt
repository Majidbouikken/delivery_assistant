package com.example.deliveryassistant.view.widgets
...
internal class BadgeDrawable(val context: Context) : Drawable() {
    ...
    private val textPaint by lazy {
        TextPaint().apply {
            isAntiAlias = true
            color = Color.WHITE
            textSize = context.resources.getDimension(R.dimen.bottom_navigation_bar_badge_text_size)
            isFakeBoldText = true
    ...
    fun updateBadgeBounds(parentBounds: Rect) {
        ...
        val size = if (count > 0) {
            context.resources.getDimensionPixelSize(R.dimen.bottom_navigation_bar_badge_size)
        } else {
            context.resources.getDimensionPixelSize(R.dimen.bottom_navigation_bar_badge_size_numberless)
        }
